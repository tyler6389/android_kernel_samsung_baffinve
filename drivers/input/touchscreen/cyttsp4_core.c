...
u16 mem_menu_sensitivity;
u16 mem_back_sensitivity;

EXPORT_SYMBOL(mem_menu_sensitivity);
EXPORT_SYMBOL(mem_back_sensitivity);
#endif

bool kernel_firmware_update;
EXPORT_SYMBOL(kernel_firmware_update);
...
static int touchkey_led_on_cmd_reserved;
...
static ssize_t set_firm_update(struct device *dev,
...
#else
	return sprintf(buf, "CY%02X%04X\n", SAMSUNG_HW_VERSION, SAMSUNG_FW_VERSION);
#endif
}
		...
		size_t size)
{
	...
	if (cd->sleep_state != SS_SLEEP_OFF
		...
		touchkey_led_on_cmd_reserved = 1;
		return size;
	}
...
{
	struct cyttsp4_core_data *cd = dev_get_drvdata(dev);
	u8 btn_diff_values[16], *ptr;
	s32 local_menu_sensitivity = 0;
	u32 menu_sensitivity_tmp = 0;
	int rec_size = 0;

	if (cd->sysinfo.ready) {
		rec_size = cd->sysinfo.si_ofs.btn_rec_size;
		memcpy(btn_diff_values,
			cd->sysinfo.btn_rec_data + 0 * rec_size, rec_size);
	} else {
		dev_err(cd->dev, "%s: fail read diff count\n",
			__func__);
		return 0;
	}

	menu_sensitivity_tmp = btn_diff_values[0];
	ptr = &btn_diff_values[0];
	while (--rec_size) {
		ptr++;
		menu_sensitivity_tmp <<= 8;
		menu_sensitivity_tmp |= ptr[0];
	}

	local_menu_sensitivity = (s32)menu_sensitivity_tmp;

#if defined(READ_MEM_SENSITIVITY)
	if (mem_menu_sensitivity != 0) {
		local_menu_sensitivity = mem_menu_sensitivity;
		mem_menu_sensitivity = 0;
	}
#endif
	dev_err(cd->dev, "%s: read value %d, %d\n", __func__, local_menu_sensitivity, menu_sensitivity_tmp);
...
#if defined(READ_MEM_SENSITIVITY)
	if (mem_menu_sensitivity != 0 && local_menu_sensitivity < 35) {
		local_menu_sensitivity = mem_menu_sensitivity;
		mem_menu_sensitivity = 0;
	}
#endif
	printk(KERN_INFO "%s: read value %d, %d\n", __func__, local_menu_sensitivity, (s16)menu_sensitivity_tmp);
			...
			struct device_attribute *attr, char *buf)
{
	struct cyttsp4_core_data *cd = dev_get_drvdata(dev);
	u8 btn_diff_values[16], *ptr;
	s32 local_back_sensitivity = 0;
	u32 back_sensitivity_tmp = 0;
	int rec_size = 0;

	if (cd->sysinfo.ready) {
		rec_size = cd->sysinfo.si_ofs.btn_rec_size;
		memcpy(btn_diff_values,
			cd->sysinfo.btn_rec_data + 1 * rec_size, rec_size);
	} else {
		dev_err(cd->dev, "%s: fail read diff count\n",
			__func__);
		return 0;
	...
	back_sensitivity_tmp = btn_diff_values[0];
	ptr = &btn_diff_values[0];
	while (--rec_size) {
		ptr++;
		back_sensitivity_tmp <<= 8;
		back_sensitivity_tmp |= ptr[0];
	}

	local_back_sensitivity = (s32)back_sensitivity_tmp;

#if defined(READ_MEM_SENSITIVITY)
	if (mem_back_sensitivity != 0) {
		local_back_sensitivity = mem_back_sensitivity;
		mem_back_sensitivity = 0;
	}
#endif
	dev_err(cd->dev, "%s: read value %d, %d\n", __func__, local_back_sensitivity, back_sensitivity_tmp);
...
#if defined(READ_MEM_SENSITIVITY)
	if (mem_back_sensitivity != 0 && local_back_sensitivity < 35) {
		local_back_sensitivity = mem_back_sensitivity;
		mem_back_sensitivity = 0;
	}
#endif
	printk(KERN_INFO "%s: read value %d, %d\n", __func__, local_back_sensitivity, (s16)back_sensitivity_tmp);
...
static DEVICE_ATTR(tsp_firm_update, S_IRUGO | S_IWUSR | S_IWGRP,
	NULL, set_firm_update);/* firmware update */
...
static int cyttsp4_core_wake_(struct cyttsp4_core_data *cd)
{
	...
	if (touchkey_led_on_cmd_reserved && cd->pdata->led_power) {
		printk(KERN_INFO "led_power on\n");
		touchkey_led_on_cmd_reserved = 0;
...
static int cyttsp4_startup_(struct cyttsp4_core_data *cd)
{
	...
	mutex_lock(&cd->system_lock);
#ifdef SAMSUNG_SYSINFO_DATA
	if (cd->sysinfo.ready && cd->sysinfo.si_ptrs.samsung_data)
		if (cd->sysinfo.si_ptrs.samsung_data->hw_version
				>= HW_VER_BL_FAST_EXIT)
...
#ifdef CONFIG_HAS_EARLYSUSPEND
	cd->early_suspend.level = EARLY_SUSPEND_LEVEL_STOP_DRAWING - 1;
