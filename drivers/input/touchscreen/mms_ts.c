...
static int set_conifg_flip_cover(struct mms_ts_info *data, int enables);
...
static eISCRet_t mms100_open_mbinary(struct i2c_client *_client)
{
	...
	ret += request_firmware(&(fw_mbin[0]),\
				"tsp_melfas/465GS37/BOOT.fw", &_client->dev);
	ret += request_firmware(&(fw_mbin[1]),\
				"tsp_melfas/465GS37/CORE.fw", &_client->dev);
	ret += request_firmware(&(fw_mbin[2]),\
				"tsp_melfas/465GS37/PRIV.fw", &_client->dev);
	ret += request_firmware(&(fw_mbin[3]),\
				"tsp_melfas/465GS37/PUBL.fw", &_client->dev);
#else
	ret += request_firmware(&(fw_mbin[1]),\
...
#if defined(FW_465GS37) && defined(ISP_UPDATE)
...
static int mms_ts_fw_load(struct mms_ts_info *info)
{
	...
	while (retries--) {
		...
		ver = get_fw_version(info);
		info->fw_ic_ver = ver;
		ver = get_hw_version(info);
		info->fw_hw_ver = ver;

		if (ret == 0) {
			pr_err("[TSP] mms100_ISC_download_mbinary success");
#ifdef FW_465GS37
			if (info->fw_ic_ver < FW_VERSION_4_8)
				continue;
#endif
			goto done;
		} else {
...
static void get_config_ver(void *device_data)
{
	...
	snprintf(buff, sizeof(buff), "E220S_Me_1222");
#else
...
static int set_conifg_flip_cover(struct mms_ts_info *info, int enables)
{
	...
	if (info->enabled) {
		if (enables) {
			dev_notice(&client->dev, "flip enable!\n");
			i2c_smbus_write_byte_data(info->client, 0x35, 0x1);
		} else {
			dev_notice(&client->dev, "flip disable!\n");
			i2c_smbus_write_byte_data(info->client, 0x35, 0x2);
		}
	}
	return 0;
}
...
static void flip_cover_enable(void *device_data)
{
	struct mms_ts_info *data = (struct mms_ts_info *)device_data;
	int status = 0;
	char buff[16] = {0};
