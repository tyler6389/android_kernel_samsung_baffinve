...
static ssize_t
lightsensor_raw_data_store(struct device *dev, struct device_attribute *attr,
		   const char *buf, size_t count)
{
	struct sensor_data *data = dev_get_drvdata(dev);
	int value;
...
static DEVICE_ATTR(raw_data, 0664, lightsensor_raw_data_show, lightsensor_raw_data_store);
