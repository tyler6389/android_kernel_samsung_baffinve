...
#ifdef CONFIG_MACH_BAFFIN /* for read i2c reg */
struct mutex gp2a_mutex;
#endif


#ifdef GP2A_PROXIMITY_ESD_SUPPORT
#define LIMIT_RESET_COUNT	1000
int proximity_esd_state = 0; /* 0=normal, 1=detected, 2=processing */
int reset_cnt = 0;
...
#ifdef GP2A_PROXIMITY_ESD_SUPPORT	
	struct delayed_work work_prox_esd;
#endif
	int enabled;
	int proximity_data;
	int irq;
...
#ifdef GP2A_CAL_BY_LON
            if (new_threshold > data->thresh_diff)
...
#ifdef GP2A_CAL_BY_LON
            if (new_threshold > data->thresh_diff)
                set_value = ((new_threshold-data->thresh_diff) & 0xFF00) >> 8;
            else
                set_value = 0;
#else
		       ...
		       const char *buf, size_t count)
{
	...
	if (data->enabled && !value) {	/* Proximity power off */
#ifdef GP2A_PROXIMITY_ESD_SUPPORT			
		cancel_delayed_work_sync(&data->work_prox_esd);
#endif	
		...
		enable_irq(data->irq);
#ifdef GP2A_PROXIMITY_ESD_SUPPORT
		queue_delayed_work(data->prox_wq, &data->work_prox_esd,
				   msecs_to_jiffies(GP2A_PROX_ESD_MS));
#endif
	}
	data->enabled = value;
...
#ifdef GP2A_CAL_BY_LON
	for (i = 0; i < COL; i++) {
		if (selected_image[i][0] == PS_HIGH_THD_L)
			/*PS mode LTH(Loff) */
			threshold = selected_image[i][1];
		else if (selected_image[i][0] == PS_HIGH_THD_H)
			/*PS mode LTH(Loff) */
			threshold |= selected_image[i][1]<<8;
	}
#else
	for (i = 0; i < COL; i++) {
		if (selected_image[i][0] == PS_LOW_THD_L)
			/*PS mode LTH(Loff) */
			threshold = selected_image[i][1];
		else if (selected_image[i][0] == PS_LOW_THD_H)
			/*PS mode LTH(Loff) */
			threshold |= selected_image[i][1]<<8;
	}
#endif
...
static int proximity_open_calibration(struct gp2a_data *data)
{
	...
	if (data->cal_data != 0) {/*If there is an offset cal data. */
		if (is_gp2a030a())
			gp2a_original_image_030a[5][1] = 8 + data->cal_data; /* CAL=8+crosstalk_ADC */
		else
			gp2a_original_image[5][1] = 8 + data->cal_data;
...
static int proximity_store_calibration(struct device *dev, bool do_calib)
{
	...
	if (do_calib) {
		...
		/* CAL=8+crosstalk_ADC, 8 is gp2a fixed value */
		selected_image[5][1] = 8 + gp2a->cal_data;
	} else { /* reset */
		gp2a->cal_data = 0;
		selected_image[5][1] = gp2a->default_threshold;
	}

	/* Update cal data. */
	gp2a_update_threshold(gp2a,selected_image,
			selected_image[5][1], true);

	pr_info("%s: prox_cal = %d, prox_thresh = 0x%x\n",
		__func__, gp2a->cal_data,
			selected_image[5][1]);
#else /* GP2A_CAL_BY_LON -------------------------*/
	if (do_calib) {
		gp2a->cal_data = proximity_adc_read(gp2a);
		selected_image[3][1] += gp2a->cal_data;
	} else { /* reset */
		gp2a->cal_data = 0;
		selected_image[3][1] = gp2a->default_threshold;
	}

	/* Update cal data. */
	gp2a_update_threshold(gp2a, selected_image,
			selected_image[3][1], true);

	pr_info("%s: prox_cal = %d, prox_thresh = 0x%x\n",
		__func__, gp2a->cal_data,
			selected_image[3][1]);
#endif /* GP2A_CAL_BY_LON -------------------------*/
	old_fs = get_fs();
	set_fs(KERNEL_DS);
...
#ifdef GP2A_PROXIMITY_ESD_SUPPORT
/***************************************************/
static void gp2a_work_func_prox_esd(struct work_struct *work)
{
	struct gp2a_data *gp2a = container_of((struct delayed_work *)work,
					      struct gp2a_data, work_prox_esd);
						
	unsigned char get_D2_data[2] = { 0, };
	int proximity_value = 0;
	static int prox_esd_cnt = 0;

	if (!gp2a->enabled)
		goto return_work_func;

	if (proximity_esd_state == 1) { /* esd detected */
		gprintk("proximity_esd_state=1, wait light sensor reset.\n");				
		if (reset_cnt++ <= LIMIT_RESET_COUNT) {
			lightsensor_reset(0);
			lightsensor_reset(1);
			pr_info("%s : lightsensor reset done. proxi processing\n",
					__func__);
		} else {
			reset_cnt = LIMIT_RESET_COUNT + 1;
		}
		proximity_esd_state = 2;
		
		goto exit_work_func;
	}

	if (proximity_esd_state == 2) { /* esd processing */
		gprintk("proximity_esd_state=2, processing proximity reset.\n");

		proximity_onoff(0);
		proximity_onoff(1);
		
		proximity_esd_state = 0;
		reset_cnt = 0;
		goto exit_work_func;
	}
	...
	if (proximity_value == 0 || proximity_value > 1024) {
		if (++prox_esd_cnt >= 2) {
			proximity_esd_state = 1;
			prox_esd_cnt = 0;
		}
	} else {
		proximity_esd_state = 0;
...
exit_work_func:
	if (gp2a->enabled) {
		queue_delayed_work(gp2a->prox_wq, &gp2a->work_prox_esd,
			msecs_to_jiffies(GP2A_PROX_ESD_MS));
	}
...
#ifdef GP2A_PROXIMITY_ESD_SUPPORT		
	INIT_DELAYED_WORK(&gp2a->work_prox_esd, gp2a_work_func_prox_esd);
#endif
...
#ifdef GP2A_PROXIMITY_ESD_SUPPORT			
	cancel_delayed_work_sync(&gp2a->work_prox_esd);
#endif	
	destroy_workqueue(gp2a->prox_wq);
#ifdef CONFIG_SLP
...
#ifdef GP2A_PROXIMITY_ESD_SUPPORT
int lightsensor_reset(u8 onoff)
{
	u8 value = 0;
	int i = 0;

#ifdef DEBUG
	gprintk("lightsensor_reset = %d\n", onoff);
	...
	if (onoff) {
		gprintk("lightsensor_reset - Light sensor reset start.\n");
		value = 0x01; /*shutdown mode */
		...
		gprintk("lightsensor_reset - Light sensor reset done.\n");
