...
#ifdef CONFIG_MACH_BAFFIN_KOR_LGT
#define MAX_PWM (190)
#else
#define MAX_PWM (185)
#endif
...
*/
	// check sleep mode (bit 4)
	if (ddi_status[1]&0x10) {
...
static int lms501xx_set_cabc(struct lcd_info *lcd)
{
	...
	if (lcd->current_cabc || lcd->siop_enable)
	{
		...
		dev_info(&lcd->ld->dev, "%s - on\n", __func__);
	}
	else
	{
		lms501xx_write(lcd,
			SEQ_SET_CABC_OFF, ARRAY_SIZE(SEQ_SET_CABC_OFF));
		dev_info(&lcd->ld->dev, "%s - off\n", __func__);
	}
...
static int update_brightness(struct lcd_info *lcd, u8 force)
{
	...
	if (lcd->auto_brightness)
		lcd->current_cabc = 1;
	else
		lcd->current_cabc = 0;

	lcd->bl = get_backlight_level_from_brightness(brightness);
	lcd->candela = convert_brightness(lcd, brightness);
...
static int lms501xx_get_brightness(struct backlight_device *bd)
{
	...
	return lcd->bd->props.brightness;
}
