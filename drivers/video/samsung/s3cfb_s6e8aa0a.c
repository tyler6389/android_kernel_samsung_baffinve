...
void esd_toggle_hs_clock(struct lcd_info *lcd)
{
	unsigned int reg = (readl(lcd->dsim->reg_base + S5P_DSIM_CLKCTRL)) &
...
{
	unsigned char	ddi_status[DDI_STATUS_LEN] = {0,};
	...
	s6e8ax0_read_ddi_status_reg(lcd, ddi_status);

	if (0x9c == ddi_status[0]) {/*0x9c*/
		/*printk(KERN_INFO "%s,
...
write_retry:
	...
	if (!ret) {
		if (retry_cnt) {
			...
			printk(KERN_ERR "[ERROR:LCD] %s : write failed\n", __func__);
	}
...
static int update_brightness(struct lcd_info *lcd, u8 force)
{
	...
	if ((force) || ((lcd->ldi_enable) && (lcd->current_bl != lcd->bl))) {
		...
		dev_info(&lcd->ld->dev, "brightness=%d, bl=%d, candela=%d (%d %d %d %d)\n",
			brightness, lcd->bl, candela_table[lcd->bl], lcd->auto_brightness, lcd->current_acl, lcd->acl_enable, lcd->siop_enable);
	}
...
static int cam_on = 0;
static int siop_on = 0;
void set_siop_enable(void)
{
	int value=0;
	struct lcd_info *lcd = g_lcd;

	if(lcd) value = lcd->siop_enable;
	if(cam_on && siop_on) value=1;
	if(!cam_on && !siop_on) value=0;

	if (lcd && lcd->siop_enable != value) {
